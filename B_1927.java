...
//			 while(now>0 && Math.abs(a.get(now)) <= Math.abs(a.get((now-1)/2)) && a.get(now)<Math.abs(a.get((now-1)/2))) //현재 위치가 root가 아니고, 부모 노드보다 현재 노드가 더 클 때 
//			 { 
...
//				 if(now*2+2 > a.size())
//					 break;
//				if(now*2+2 == a.size() || a.get(now*2+1)<a.get(now*2+2)) //왼쪽이 오른쪽보다 더 작고 오른쪽이 존재할 때
//				{
//					if(a.get(now) > a.get(now*2+1) && Math.abs(now)<= Math.abs(now*2+1)) //자식 왼쪽값이랑 비교해서 부모가 더 클경우
//					{
//						//값 change
//
...
//				else 
//				{
//					if(a.get(now) > a.get(now*2+2)&& Math.abs(now)<= Math.abs(now*2+2)) //오른쪽값과 비교
//					 {
//						//값 change
