		 ...
		 else //Delete 가능
		 {
			 ...
			 { 
				 if(now*2+2 > a.size()) //overflow
					 break;
				if(now*2+2 == a.size() || a.get(now*2+1)>a.get(now*2+2)) //왼쪽 혼자거나 오른쪽보다 왼쪽이 클 때
				{
					if(a.get(now) < a.get(now*2+1)) //자식 왼쪽값이랑 비교해서 부모가 작을 클경우
					{
						//값 change
				...
				{
					if(a.get(now) < a.get(now*2+2)) //오른쪽값과 비교
					 {
						//값 change
